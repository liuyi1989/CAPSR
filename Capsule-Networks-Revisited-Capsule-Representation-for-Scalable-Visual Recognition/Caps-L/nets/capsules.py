...
class ConvCaps(nn.Module):
    ...
    def __init__(self, B=32, C=32, K=3, P=4, stride=2, iters=3, channel1 = 272,
        ...
        self.conv1 = BasicConv2d_activation(C*K*K*16, C*K*K*1, 3)       
        self.conv2 = BasicConv2d_activationRL(C*K*K*17, C*17, 3)   
...
class CapsNet(nn.Module):
    ...
    Args:
        ...
        ...
    """
    def __init__(self, A=32,A1=32,A2= 32, B1=32,B2=32, C1=32, C2=32, D1=32, D2=32, E=10, K=3, P=4, iters=3):
        super(CapsNet, self).__init__()
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=A,
                               kernel_size=3, stride=1, padding=1)
        self.bn1 = nn.BatchNorm2d(num_features=A, eps=0.001,
                                 momentum=0.01, affine=True)
        self.bn2 = nn.BatchNorm2d(num_features=A2*17, eps=0.001,
                                 momentum=0.01, affine=True)
        self.bn3 = nn.BatchNorm2d(num_features=B2*17, eps=0.001,
                                 momentum=0.01, affine=True)
        self.bn4 = nn.BatchNorm2d(num_features=C2*17, eps=0.001,
                                 momentum=0.01, affine=True)      
        self.bn5 = nn.BatchNorm2d(num_features=D2*17, eps=0.001,
                                 momentum=0.01, affine=True)   
        # self.bn6 = nn.BatchNorm2d(num_features=G*17, eps=0.001,
        #                          momentum=0.1, affine=True)        
        self.relu1 = nn.ReLU(inplace=False)
        self.primary_caps1 = PrimaryCaps(A, A, K, P, stride=2)
        self.primary_caps2 = PrimaryCaps(A2*17, A2, K, P, stride=2)
        self.primary_caps3 = PrimaryCaps(B2*17, B2, K, P, stride=2)
        self.primary_caps4 = PrimaryCaps(C2*17, C2, K, P, stride=2)
        #self.primary_caps5 = PrimaryCaps(FF*17, E, 1, P, stride=1)
        # self.primary_caps5 = PrimaryCapsClass(D2*17, E, 1, P, stride=1, h=2, w=2)
        self.conv_caps1_1 = ConvCaps(A, A1, K, P, stride=1, iters=iters)
        self.conv_caps1_2 = ConvCaps(A1, A2, K, P, stride=1, iters=iters)
        self.conv_caps2_1 = ConvCaps(A2, B1, K, P, stride=1, iters=iters)
        self.conv_caps2_2 = ConvCaps(B1, B2, K, P, stride=1, iters=iters)
        self.conv_caps3_1 = ConvCaps(B2, C1, K, P, stride=1, iters=iters)
        self.conv_caps3_2 = ConvCaps(C1, C2, K, P, stride=1, iters=iters)
        self.conv_caps4_1 = ConvCaps(C2, D1, K, P, stride=1, iters=iters)
        self.conv_caps4_2 = ConvCaps(D1, D2, K, P, stride=1, iters=iters)
        # self.class_caps = ConvCaps(E, E, 1, P, stride=1, iters=iters,
        #                                 coor_add=True, w_shared=True) 
        # self.convd = RF(A, A)
        self.fc1 = torch.nn.Sequential(Conv(D2*17, 1280, torch.nn.SiLU()),
                                       torch.nn.AdaptiveAvgPool2d(1),
                                       torch.nn.Flatten())
        self.fc2 = torch.nn.Linear(1280, 1000)
        self.gelu4 = nn.GELU()
        self.gelu2 = nn.GELU()
        self.gelu3 = nn.GELU()
        self.gelu5 = nn.GELU()
