...
class ConvCaps(nn.Module):
    ...
    def __init__(self, B=32, C=32, K=3, P=4, stride=2, iters=3, channel1 = 272,
        ...
        self.weights = nn.Parameter(torch.randn(1,1,C,B))
        # op
        self.sigmoid = nn.Sigmoid()
        ...
        self.gamma = nn.Parameter(torch.ones(1, 1, 1,C,1 ))
    ...
    def transform_view(self, p,a, w, P,b,oh,ow, w_shared=False):
        ...
        # print(p.shape)
        p = p.permute(0,3,1,2,4).reshape(b,self.B,-1)
        # print("23123123",p.shape)
        ...
        p = p.permute(0,2,1).reshape(b,oh*ow, self.K*self.K,self.C, self.psize)
        # print(p.shape)
        # exit()
        p = p*self.gamma
        # print("p",p.shape)
        # psum = torch.sum(p, dim=3, keepdim=True) #[128, 256, 9, 1, 16]
